...
$sitemapIndexHeader = '
<?xml version="1.0" encoding="UTF-8"?>' . $generatorInfo . '
...
for ($i = 0; $i < sizeof($this->sitemaps); $i++) {
    ...
    */
    ...
    //$robotsFileContent .= "Sitemap: $site_url/sitemap.xml";
    ...
    */
    ...
    $searchEngines[0] = isset($yahooAppId) ? str_replace("USERID", $yahooAppId, $searchEngines[0][0]) :
    $searchEngines[0][1];
    $result = array();
    for ($i = 0; $i < sizeof($searchEngines); $i++) { $submitSite=curl_init($searchEngines[$i] .
        htmlspecialchars($this->sitemapFullURL, ENT_QUOTES, 'UTF-8'));
